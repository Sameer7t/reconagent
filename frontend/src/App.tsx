  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      .then(([caseRes, invRes]) => {
        ...
        let agentConclusion = '';
        if (hasCalculationError) {
          const docName = normalizedThreeWay.receipt_id || normalizedThreeWay.invoice_id || 'document';
          agentConclusion = `Document arithmetic error on ${docName}. The printed total does not equal the itemized calculation, so the invoice cannot be processed. Recommended action: Reject invoice and request a corrected document from ${normalizedThreeWay.vendor_name || 'the vendor'}.`;
          ...
          dynamicSteps = [
            {
              ...
              findings: `Audited arithmetic on uploaded documents. Found calculation discrepancy where printed total does not match itemized lines.`,
            },
            {
              step: 2,
              action: 'Investigation Decision',
              tool: 'Validation Policy',
              findings: `Stopped automated investigation immediately because the document is mathematically incorrect. No agent queries run on invalid documents.`,
            },
            {
              step: 3,
